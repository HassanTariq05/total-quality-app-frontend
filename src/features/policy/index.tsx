...
export function PolicyVersionsView() {
  ...
  const canViewPolicyVersions = useHasPermission(
    PERMISSIONS.VIEW_POLICY_VERSION
  )

  const canClonePolicy = useHasPermission(PERMISSIONS.CLONE_POLICY_VERSION)
    ...
    <PolicyVersionsProvider>
      ...
      <Main className='flex flex-1 flex-col gap-6'>
        ...
        {isLoadingVersions ? (
          ...
          <>{canViewPolicyVersions && <PolicyVersions data={versions} />}</>
        )}
      </Main>
